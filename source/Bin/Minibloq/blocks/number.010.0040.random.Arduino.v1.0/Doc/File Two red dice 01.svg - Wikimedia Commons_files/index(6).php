...
/*
  THIS IS ImageAnnotator VERSION 2.2b
...
ImageAnnotation.getRevisionId = function (htmltextOfWikiPage) {
  ...
  if (window.mediaWiki) { // MW 1.17+
    revision_id = htmltextOfWikiPage.match (/mediaWiki.config.set\(\{.*"wgCurRevisionId"\s*:\s*(\d+),/);
  } else { // MW < 1.17
          ...
          LAPI.Ajax.submitEdit (
            ...
            , function (request) {
                if (edit_page.isFake && (typeof (edit_page.dispose) == 'function'))
                  edit_page.dispose ();
                var revision_id = ImageAnnotation.getRevisionId (request.responseText);
                if (!revision_id) {
...
var IA =
{
  ...
  // Fallback
  indication_icon : 'http://upload.wikimedia.org/wikipedia/commons/8/8a/Gtk-dialog-info-14px.png',
  ...
  install : function (config)
  {
    ...
    self.may_edit = wgArticleId > 0 && self.haveAjax && config.editingEnabled ();
