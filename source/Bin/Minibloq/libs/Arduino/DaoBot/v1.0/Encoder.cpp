...
void encoderBrake()
{
  ...
  fpl("exit Brake with counts:",leftCount); fpln(",", rightCount); 
}
...
void speedControl(byte motor, byte targetSpeed, unsigned long currentPulseDur)
{
  ...
  if(verbose) {
   // printf("motor %d targetS=%d, pulseDur=%d, PWM=%d, error=%d, delta=%d\n", motor, targetSpeed, currentPulseDur, PWM, error, speedDelta);
  }
}
  ...
  while(true) // do until completed movement or aborted by watch function 
  {
    ...
    if( millis() - startTime > MAX_UNMOVING_TIME)
    {
      ...
      { 
        ...
        fpln("speed increased to ",moveGetSpeed());
