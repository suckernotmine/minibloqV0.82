...
{
#ifndef NO_ENCODER 
   encoderMoveWatchful(distanceCM*10, duration, watchFunc);  // encoder units are MM
#else
...
{  
  ...
  Serial.print(", dist = ");  Serial.print(distanceMM);
#ifdef NO_ENCODER	
  ...
  motorReverse(MOTOR_LEFT, moveSpeed);
  motorForward(MOTOR_RIGHT, moveSpeed);
  result = MoveWatchful(distanceMM/10, dur, watchFunc);
