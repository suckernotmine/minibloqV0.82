/*
  LedStrip library based on adadfruit NeoPixel
 */ 
...
void BarGraph(uint32_t color, int percent )
{
  ...
  for(int i=0; i < numbrOfPixels; i++) {
      if(count > i)
         ...
         strip.setPixelColor(i, map(stripR, 0,MAX_BRIGHTNESS,0,brightness),
...
//lights pixels indicated by bitmap
// 0 turns pixel off, 1 lights using given color, any other digit is ignored
// for example 10000002 lights the leftmost LED, rightmost remains as previously set, other LEDs are turned off
void LedPixels(uint32_t color, uint32_t bitmap)
{
  if(isLedStripStarted == false) {
      LedStripBegin();
	  isLedStripStarted = true;
...
// instantiate an instance of the strip
Adafruit_NeoPixel strip = Adafruit_NeoPixel(numbrOfPixels, LED_STRIP_PIN, NEO_GRB + NEO_KHZ800);
